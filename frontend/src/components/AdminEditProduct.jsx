...
function AdminEditProduct({onClose, productData, onProductUpdate}) {

  const [data, setData] = useState({
    _id: productData?._id || '',
    productName: productData?.productName || '',
    brandName: productData?.brandName || 'Not Provided',
  ...
  const [view, setView] = useState(data?.productImage[0]);
  ...
  async function submitHandler(e) {
    ...
    try {
      const resp = await axios.post('/api/update-product', data, {
  ...
  return (
    <div className=" fixed top-0 bottom-0 left-0 right-0 w-screen h-screen flex items-center justify-center bg-black bg-opacity-50 z-50">
          ...
          >
            <div className="mb-4">
              ...
              <input
                ...
                className="w-full bg-transparent border-b border-gray-300 py-2 text-white text-md px-4 focus:outline-none focus:border-white truncate"
              />
            </div>
...
export default memo(AdminEditProduct)
